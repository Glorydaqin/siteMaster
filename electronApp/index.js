...
class TabGroup extends EventEmitter {
  ...
  addTab(args = this.options.newTab) {
    ...
    tab.webview.addEventListener('new-window', function (e) {
      ...
      // allTabs.push(tabGroup.addTab({
      //   title: "title",
      //   src: e.url,
      //   // visible: true,
      //   active: true
      // }));
      // this.bind(this.arguments)
      // _this.tabs.push(_this.addTab());
      // _this.addTab.call(_this, {src: e.url, active: true});
      _this.addTab({title: e.url, src: e.url, active: true})
    });
    this.tabs.push(tab);
...
const TabPrivate = {
  ...
  initWebview: function () {
    ...
    const tabWebviewDomReadyHandler = function (e) {
      ...
      //
      // var _ret = this.webview.executeJavaScript("document.title");
    ...
    var that = this;
    this.webview.addEventListener('page-title-updated', function (info) {
      // console.log(info)
